...
**Phase 1 complete (M2)** — the packet layer is implemented and its format is frozen.
`protocol/packet.py` defines the wire format for both endpoints; 251 unit tests pass.
The remaining modules under `protocol/`, `network/` and `experiments/` are documented
placeholders, each naming the task that implements it.

Frozen so far: **D1** byte layout, **D2** checksum coverage, **D3** segment size,
**D4** sequence convention. Next up is **Phase 2 (T2.1–T2.5): sender, receiver, and a
file that transfers end to end at 0% loss.**
...
receiver.py            real receiver — written in T2.2             (T2.2)
